...
public class RedeemPointFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fr_redeem, container, false);
        return rootView;
    }
    ...
    private void openNewsListPage() {
        Fragment fragment = new RedeemPointListFragment();
        if (getArguments() != null)
            fragment.setArguments(getArguments());

        if(getActivity() != null && getActivity() instanceof BaseFragmentActivity) {
            BaseFragmentActivity activity = (BaseFragmentActivity) getActivity();
            activity.swap(R.id.containerFragmentRedeem, fragment, false, false);
        }
    }
}
